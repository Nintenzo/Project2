...
def process_posts(posts_segment, average_sleep_time, previous_openings, cap=128_000_000):
    for x in posts_segment:
        if asizeof(previous_openings) > cap:
            previous_openings.clear()
...
while True:
    try:
        ...
        if len(posts) >= 1:
            ...
            mid = len(posts) // 2
            first_half = random.sample(posts[:mid], len(posts[:mid]))
            second_half = random.sample(posts[mid:], len(posts[mid:]))
            
            thread1 = threading.Thread(target=process_posts, args=(first_half, average_sleep_time, previous_openings, CAP))
            thread2 = threading.Thread(target=process_posts, args=(second_half, average_sleep_time, previous_openings, CAP))
